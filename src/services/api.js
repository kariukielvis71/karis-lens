 ...
 * @param {{ method?: string, body?: unknown, auth?: boolean, formData?: FormData }} options
 */
export async function request(path, { method = "GET", body, auth = false, formData } = {}) {
  const headers = {};
  if (auth) {
...
/** Upload a cover image to R2 via the Worker. Returns { key, url }. */
